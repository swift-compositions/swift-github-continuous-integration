...
struct CIValidationDriftTests {
    static func document(_ text: String) throws -> GitHub.ContinuousIntegration.Workflow.YAML.Node {
        try GitHub.ContinuousIntegration.Workflow.YAML.Parser.parse(text)
    }
}
...
extension CIValidationDriftTests {
    ...
    struct `Scheduled Workflow Watch List` {
        typealias Subject = GitHub.ContinuousIntegration.Validation.Drift.ScheduledWorkflowAlert
