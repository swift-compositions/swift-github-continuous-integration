...
struct CIValidationBinaryInstallChecksumTests {
    static func findings(_ script: String) -> [GitHub.ContinuousIntegration.Validation.Finding] {
        GitHub.ContinuousIntegration.Validation.BinaryInstallChecksum.findings(
            in: script,
    ...
    struct `Edge Case` {
        @Test func `the run block is the unit, not the file`() {
            ...
            let findings = try? GitHub.ContinuousIntegration.Validation.BinaryInstallChecksum()
        ...
        @Test func `a curl on one line does not borrow a flag from another`() {
            ...
            #expect(
                !GitHub.ContinuousIntegration.Validation.BinaryInstallChecksum.fetchesWithCurl(
