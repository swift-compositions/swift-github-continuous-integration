...
struct CIValidationPinnedActionSchemaTests {
    ...
    struct `Positive Control` {
        @Test func `an undeclared with key and an absent output both fire, reconstructing 500`() throws {
            ...
            let findings = try GitHub.ContinuousIntegration.Validation.PinnedActionSchema().findings(in: repository.subject)
    ...
    struct `Shallow Checkout` {
        @Test func `a pinned commit older than the shallow boundary is fetched and resolved, not falsely flagged`() throws {
            ...
            try GitHub.ContinuousIntegration.Validation.PinnedContent.spawning.sync { try clone.run() }
            clone.waitUntilExit()
            ...
            try GitHub.ContinuousIntegration.Validation.PinnedContent.spawning.sync { try probe.run() }
            probe.waitUntilExit()
            #expect(probe.terminationStatus != 0)

            let path = ".github/actions/install-example"
            let coordinate = GitHub.ContinuousIntegration.Validation.PinnedContent.Coordinate(path: path, sha: oldSHA)
            let resolution = GitHub.ContinuousIntegration.Validation.PinnedContent.resolve(coordinate, gitRoot: shallowRoot)
            switch resolution {
