...
extension GitHub.ContinuousIntegration.Validation {
    ...
    public struct CachePolicy: Validator {
        ...
        static func buildCacheMessage(document: String, job: String, path: String) -> String {
            """
            \(document): job \(GitHub.ContinuousIntegration.Workflow.YAML.Node.repr(job)) caches `.build/` via \
        ...
        static func restoreKeysMessage(document: String, job: String, preview: String) -> String {
            """
            \(document): job \(GitHub.ContinuousIntegration.Workflow.YAML.Node.repr(job)) cache step uses \
